...
# Clean the packet summary for display keeping only the human readable parts
...
# Check if a packet matches the selected filter including the ip address and the destination port
...
# Show full detailed info of the selected packet with layer-by-layer display
def show_detailed_info():
    ...
    detail_window.title("Packet Details (Layer-by-Layer)")
    detail_window.geometry("800x600")

    text_area = tk.Text(detail_window, wrap=tk.WORD, font=("Courier", 10), bg="black", fg="white", insertbackground="white")

    lines = []
    lines.append("Frame Summary:")
    lines.append(f"  - Arrival Time: {pkt.time - start_time:.6f} seconds since capture start")
    ...
    while current_layer:
        ...
        lines.append(f"{indent}{layer_name} Layer:")
        for field_name in current_layer.fields:
            value = current_layer.getfieldval(field_name)
            lines.append(f"{indent}  {field_name}: {value}")
        lines.append("")
        current_layer = current_layer.payload
        if not current_layer or current_layer == b'':
            break
        level += 1
    ...
    route_status.config(text="Packet details displayed (layer-by-layer).", fg="green")
